...
import { getGamesByTag } from '../services/gameService.js';

export default function TagsPage({ onFavorite, favorites }) {
  ...
  const tagNameParam = searchParams.get('name') || '';

  useEffect(() => {
    setTagName(tagNameParam);
  }, [tagNameParam]);
  ...
  useEffect(() => {
    const fetchGames = async () => {
      try {
        setLoading(true);
        const data = await getGamesByTag(tagId, currentPage);
        setGames(data.results || []);
        setTotalGames(data.count || 0);
        setError(null);
      } catch (err) {
        setError('No se pudieron cargar los juegos del tag. Por favor, intenta de nuevo.');
        console.error(err);
      } finally {
        setLoading(false);
      }
  ...
  const handlePageChange = (newPage) => {
    setSearchParams({ page: newPage.toString(), name: tagName });
    ...
    <div className="min-h-screen relative" style={{ background: '#1A1A1D' }}>
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 py-14">
        ...
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          <div>
            <div className="flex items-center gap-3 mb-3">
              ...
              >
                Tag: <span style={{ color: '#A64D79' }}>"{tagName}"</span>
              </h1>
            </div>
            <p className="text-sm pl-3.5" style={{ color: 'rgba(212,197,208,0.5)' }}>
              {totalGames > 0
                ? <>{totalGames.toLocaleString()} <span style={{ color: '#A64D79', fontWeight: 600 }}>{totalGames === 1 ? 'juego' : 'juegos'}</span> encontrados</>
        ...
        {error ? (
          ...
          <ErrorMessage message="No se encontraron juegos para este tag." />
        )}
      </div>
    </div>
  );
